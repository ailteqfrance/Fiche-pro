...
def get_product_data(asin, marketplace="fr"):
    """Scrape la fiche Amazon via ScraperAPI + parse avec BeautifulSoup."""
    url = f"https://www.amazon.{marketplace}/dp/{asin}"

    if SCRAPER_API_KEY:
        # Via ScraperAPI — contourne les blocages Amazon
        scraper_url = "https://api.scraperapi.com/"
        params = {
            "api_key": SCRAPER_API_KEY,
            "url": url,
            "country_code": marketplace if marketplace in ["fr", "de", "es", "it"] else "us",
            "render": "false",
        }
        response = requests.get(scraper_url, params=params, timeout=60)
    else:
        # Fallback direct (local uniquement)
        import random
        USER_AGENTS = [
    ...
    if "captcha" in response.text.lower() and len(response.text) < 5000:
        raise Exception("Captcha détecté — réessayez dans quelques secondes")
    ...
    # ── Note ───────────────────────────────────────────────────────────────────
    rating = None
    for selector in ["span.a-icon-alt", "#acrPopover span.a-icon-alt"]:
    ...
    if avis_tag:
        try:
            nb_avis = int(re.sub(r"[^\d]", "", avis_tag.text.split()[0]))
        except:
            pass
    ...
    for selector in [".a-price .a-offscreen", "#priceblock_ourprice", "#priceblock_dealprice"]:
        ...
        if prix_tag:
            try:
                prix_text = re.sub(r"[^\d,.]", "", prix_tag.get_text(strip=True)).replace(",", ".")
...
def audit_listing(data, mot_cle_principal=""):
    ...
    if title_len >= 150:
        ...
        titre_score += 7
        details["titre"]["items"].append({"icon": "⚠️", "type": "warn", "texte": f"Titre correct mais enrichissable ({title_len} car.)"})
    else:
        titre_score += 3
        details["titre"]["items"].append({"icon": "❌", "type": "ko", "texte": f"Titre trop court ({title_len} car.) — visez 150+"})

    marque = data.get("marque", "")
    if marque and marque.lower() in title.lower():
        titre_score += 5
    ...
    if mot_cle_principal and mot_cle_principal.lower() in title.lower():
        titre_score += 10
        details["titre"]["items"].append({"icon": "✅", "type": "ok", "texte": f"Mot-clé '{mot_cle_principal}' présent dans le titre"})
    elif mot_cle_principal:
        details["titre"]["items"].append({"icon": "❌", "type": "ko", "texte": f"Mot-clé '{mot_cle_principal}' absent du titre"})
        titre_score += 2
    ...
    if img_count >= 7:
        ...
        img_score = 12
        details["images"]["items"].append({"icon": "⚠️", "type": "warn", "texte": f"{img_count} images — ajoutez lifestyle/comparatives"})
    elif img_count >= 3:
        img_score = 7
    ...
    if description:
        ...
        if mot_cle_principal and mot_cle_principal.lower() in description.lower():
            seo_score += 3
            details["seo"]["items"].append({"icon": "✅", "type": "ok", "texte": "Mot-clé présent dans la description"})
    else:
    ...
    if rating:
        if rating >= 4.5:
            ...
            preuve_score += 2
            details["preuve_sociale"]["items"].append({"icon": "⚠️", "type": "warn", "texte": f"Note moyenne : {rating}/5"})
        else:
    ...
    if details["images"]["score"] < 14:
        top3.append("Atteindre 7 images + ajouter le A+ Content")
